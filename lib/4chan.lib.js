 ...
 */
const fetchThread = (pathname = document.location.pathname) => {
...
/** Saves a thread in session storage. */
const saveThread = (thread) => {
  const threadNo = thread.posts[0].no;
  sessionStorage.setItem(threadNo, JSON.stringify(thread));
};

/** Loads a thread from session storage. */
const loadThread = threadNo => JSON.parse(sessionStorage.getItem(threadNo));
